...
import { InterfaceExpression, InterfaceMember, Reference, TypeDeclaration } from "@alloy-js/typescript";

interface Props {
  inter: Interface;
}
export function InterfaceContent({ inter }: Props) {
  const refKeyPrefix = inter.namespace ? `${getNamespaceFullName(inter.namespace)}.${inter.name}` : inter.name;
  return (
    <List hardline>
